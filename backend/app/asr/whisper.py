"""
Hugging Face Inference Providers Whisper ASR Provider.
"""
...
class WhisperProvider(ASRProvider):
    ...
    async def transcribe(
        ...
        if not self.is_available():
            raise ASRProviderUnavailableError("Whisper/HF provider is disabled or missing HF_TOKEN.")
        ...
        try:
            ...
            raise ASRProviderUnavailableError("Whisper/HF API request timed out.")
        except Exception as e:
            raise ASRFailedError(f"Network error calling Whisper/HF API: {str(e)}")

        if response.status_code == 429:
            raise ASRRateLimitedError("Hugging Face API rate limit reached.")
        elif response.status_code in (401, 403):
            raise ASRProviderUnavailableError("Hugging Face token invalid or unauthorized.")
        elif response.status_code == 503:
            raise ASRProviderUnavailableError("Model is currently loading or unavailable on HF Inference.")
        elif response.status_code >= 500:
            raise ASRProviderUnavailableError(f"Hugging Face server error: {response.status_code}")
        elif response.status_code != 200:
            raise ASRFailedError(f"Hugging Face returned status {response.status_code}: {response.text}")
